...
| `--setting-sources <list>` | какие файлы настроек читать (`user,project,local`); `""` — никакие, вместе с ними не грузятся и хуки пользователя |
| `--bare` | минимальный режим (в т. ч. без хуков), но авторизация только через `ANTHROPIC_API_KEY` — подписка не работает, поэтому не используем |
...
⚠️ Важно: хуки пользователя из `~/.claude/settings.json` действуют и в подпроцессе.
Хук `MessageDisplay` переписывает текст ответа, то есть перевод, а его таймаут
прибавляется к каждому сообщению треда. Отключаются они только вместе со всеми
пользовательскими настройками — `--setting-sources ""`; на авторизацию это не влияет,
она не из файлов настроек. `--bare` для этого не годится: он запрещает OAuth
и требует `ANTHROPIC_API_KEY`.
...
- `--setting-sources ""`: хуки пользователя не запускаются, подписочная авторизация цела.
